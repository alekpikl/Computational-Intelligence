...
int main(int argc, char* argv[]) {
    ...
    int index = 0;
    while (scanf(" %lf,%lf,%lf,%lf,%lf,%lf", &Robot[index].X_r, &Robot[index].Y_r, &Robot[index].Theta_r, &Robot[index].X_c, &Robot[index].Y_c, &Robot[index].Theta_c) != EOF) {
        ...
        printf("%lf,%lf\n",Output[index].vl,Output[index].vr);
        ++index;
    }    
    return 0;
}
