...
#define ACTIVATION_FUNCTION 2    // function: 0 - linear, 1 - logistic, 2 - tanh
...
float weights[3] = {0.0, 0.0, 0.0}; // two weights - x and bias
...
void recreate(float input[], float output[], int index) {
    ...
    if (ACTIVATION_FUNCTION == 0) {
        activation = input[0] * weights[0] + input[1] * weights[1] + weights[2];
        if (activation >= 0.5) {
            //printf("+1\n");
            output[index] = 1;}
        else {
    ...
    if (ACTIVATION_FUNCTION == 1) {
        activation = sigmoid(input[0] * weights[0] + input[1] * weights[1] + weights[2]);
        if (activation >= 0.5) {
            //printf("+1\n");
            output[index] = 1;}
        else {
    ...
    if (ACTIVATION_FUNCTION == 2) {
        activation = tanhf(input[0] * weights[0] + input[1] * weights[1] + weights[2]);
        if (activation >= 0.0) {
            //printf("+1\n");
            output[index] = 1;
        }
...
void train(MATRIX table, int numberOfExamples) {
    ...
    for (int i = 0; i < numberOfExamples; ++i) {
        ...
        //printf("Activation: %f\n", activation);
        if (proofEquality(table[i][2], 1.0)) {
            calcDelta(1.0);
        } else {
            if (ACTIVATION_FUNCTION == 2)
                calcDelta(-1.0);
...
void getActivation(float input[]) {
    float netInput = input[0] * weights[0] + input[1] * weights[1] + weights[2];
    if (ACTIVATION_FUNCTION == 0)
        activation = netInput;
...
void updateWeights(float input[]) {
    ...
    weights[2] = weights[2] - LEARNING_RATE * delta * 1;
}
...
int main(int argc, char** argv) {
    ...
    MATRIX data = createMatrix(1000, 3);
    ...
    //printCSV(data, numberOfExamples, 3);
    normalizeInput(data, numberOfExamples);
    //printCSV(data, numberOfExamples, 3);
    ...
    // Recreation
    // float in[2];
    MATRIX in = createMatrix(1, 2);
    while (scanf("%f,%f", &in[0][0], &in[0][1])>0) {
        normalizeInput(in, 1);
        activation = tanhf(in[0][0] * weights[0] + in[0][1] * weights[1] + weights[2]);
        if (activation >= 0.0)
            printf("+1\n");
        else
            printf("-1\n");
